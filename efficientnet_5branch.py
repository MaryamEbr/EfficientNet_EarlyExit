...
class EfficientNet_final(nn.Module):
    def __init__(self, inverted_residual_setting, dropout, stochastic_depth_prob, num_classes, selected_exits):
        super().__init__()
        ...
        self.layers_dict['block_br1'] = Conv2dNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=nn.BatchNorm2d, activation_layer=nn.SiLU)
        
    
        # building inverted residual blocks
        total_stage_blocks = sum(cnf.num_layers for cnf in inverted_residual_setting)
        stage_block_id = 0
        # print("len inverted_residual_setting", len(inverted_residual_setting))
        for ind, cnf in enumerate(inverted_residual_setting):
            if ind <= self.selected_exits[-1]+1:
                ...
                for _ in range(cnf.num_layers):
                    ...
                    stage.append(MBConv(block_cnf, sd_prob, nn.BatchNorm2d))
                stage_block_id += 1
                ...
                if ind == len(inverted_residual_setting)-1:
                    ...
                    # print("in init block ", 5)
                    self.layers_dict[f'block_br5'] = nn.Sequential(*stage, Conv2dNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=nn.BatchNorm2d, activation_layer=nn.SiLU))
            
                elif ind in [0, 1, 2]:
                    # print("in init block ", 1)
                    self.layers_dict[f'block_br1'].append(nn.Sequential(*stage))

                else:
                    # print("in init block ", ind-1)
                    self.layers_dict[f'block_br{ind-1}'] = nn.Sequential(*stage)


        
        # # second to last exit braches
        # print("selected exits", self.selected_exits)
        for ind, cnf in enumerate(inverted_residual_setting):
            if ind in [0, 1]:
                continue
            ...
            if ind-1 in self.selected_exits:
                ...
                self.layers_dict[f'exit_br{ind-1}'] = nn.Sequential(nn.AdaptiveAvgPool2d(1), 
                        nn.Flatten(1),
...
class EfficientNet_partial(nn.Module):
    def __init__(self, inverted_residual_setting, dropout, stochastic_depth_prob, num_classes, partition):
        super().__init__()
        ...
        for ind, cnf in enumerate(inverted_residual_setting):
            if self.partition_begin <= ind+2 <= self.partition_end:
                ...
                if ind == len(inverted_residual_setting)-1:
                    ...
                    self.layers_dict[f'block_br{ind+2}'] = nn.Sequential(*stage, Conv2dNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=nn.BatchNorm2d, activation_layer=nn.SiLU))
